...
// 	std::vector<float> PosToAngles(float x, float y, char headAngle=90.0) {
...
// 			bx = x1 < x2 ? x1 : x2;
// 			by = y1 > y2 ? y1 : y2;
// 			pathLength = getBezierPathLength(x1, y1, bx, by, x2, y2);
// 		}
...
// 		}
// 		//else {
// 		//	return getLinearPath(x1, y1, x2, y2, ha1, ha2);
// 		//}
		
// 		int positionErrorCount = 0;
// 		bool positionPossible = true;
// 		std::vector<std::vector<int>> path;
// 		//float pathLength = getBezierPathLength(x1, y1, bx, by, x2, y2);
...
// 			std::vector<float> angles = PosToAngles(x, y, ha);
// 			std::vector<int> servoValues = angleToServoValue(angles);
// 			positionPossible = constraint(servoValues, constraints_min, constraints_max, false);
// 			if (!positionPossible) positionErrorCount++;

// 			path.push_back(servoValues);
// 			//path.push_back({ x, y, ha });
// 		}
...
// 			//}
// 			result.push_back(roundf((a * 1227.6f) / 360) + defaultValues[c]);
// 			c++;
// 		}
// 	}
...
// 	float getBezierPathLength(float x1, float y1, float bx, float by, float x2, float y2) {
// 		float xOld = x1;
// 		float yOld = y1;
// 		float length = 0;
